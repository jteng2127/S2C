...
def do_python_eval(
    predict_folder, gt_folder, name_list, num_cls, task, threshold, printlog=False
):
    TP = []
    P = []
    T = []
    ...
    for i in range(8):
        p = multiprocessing.Process(
            target=compare, args=(i, 8, TP, P, T, task, threshold)
        )
        p.start()
        p_list.append(p)
    for p in p_list:
...
def eval_in_script(
    ...
    task="cam",
    pred_dir=None,
    gt_dir=os.path.join(VOC2012_DIR, "SegmentationClass"),
):
    eval_list = (
        os.path.join(VOC2012_DIR, "ImageSets/Segmentation/") + eval_list + ".txt"
    )
    ...
    if task == "cam":
        ...
        for i in range(50):
            ...
            loglist = do_python_eval(
                pred_dir, gt_dir, name_list, 21, task, t, printlog=False
            )
        ...
        loglist = do_python_eval(
            pred_dir, gt_dir, name_list, 21, task, 0, printlog=False
        )
        miou = loglist["mIoU"]
        precision = loglist["mP"]
        recall = loglist["mR"]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gt_dir", default=os.path.join(VOC2012_DIR, "SegmentationClass"), type=str
    )
    ...
    eval_list = (
        os.path.join(VOC2012_DIR, "ImageSets/Segmentation/") + args.list + ".txt"
    )
    df = pd.read_csv(eval_list, names=["filename"])
    name_list = df["filename"].values
    ...
    if args.task == "cam":
        for i in range(30):
            ...
            print(t)
            loglist = do_python_eval(
                pred_dir, args.gt_dir, name_list, 21, args.task, t, printlog=True
            )
            print(loglist)
        ...
        loglist = do_python_eval(
            pred_dir, args.gt_dir, name_list, 21, args.task, 0, printlog=True
        )
