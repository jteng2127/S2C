...
class model_WSSS:
    ...
    def update(self, epo, iter):
        ...
        if iter % 300 == 0:
            with torch.no_grad():
                ...
                cam_vis = F.interpolate(
                    cam_vis, (H, W), mode="bilinear", align_corners=False
                )
                ...
                for i in range(num_vis):
                    img_cam[i] = torch.from_numpy(
                        cam_on_image(img_vis[i], cam_vis[i, 0])
                    )
    ...
    @torch.no_grad()
    def mvweight(self):
        for param_main, param_sup in zip(
            self.net_main.parameters(), self.net_sup.parameters()
        ):
