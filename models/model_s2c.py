...
class model_WSSS:
    ...
    def __init__(self, args, logger=None, comet_logger=None):
        ...
        self.net_main.load_state_dict(
            resnet38d.convert_mxnet_to_torch(
                os.path.join(os.getenv("PRETRAINED_DIR"), "resnet_38d.params")
            ),
            strict=False,
        )
    ...
    def save_model(self, epo, ckpt_path):
        ...
        torch.save(
            self.net_main.module.state_dict(),
            ckpt_path + "/" + epo_str + "net_main.pth",
        )
    ...
    def load_model(self, epo, ckpt_path):
        epo_str = str(epo).zfill(3)
        self.net_main.load_state_dict(
            torch.load(ckpt_path + "/" + epo_str + "net_main.pth"), strict=True
        )
    ...
    def update(self, epo, iter):
        ...
        if use_cpm:
            ...
            with torch.no_grad():
                self.net_main.eval()
                img_05 = F.interpolate(
                    self.img, scale_factor=0.5, mode="bilinear", align_corners=True
                )
                img_10 = self.img
                img_15 = F.interpolate(
                    self.img, scale_factor=1.5, mode="bilinear", align_corners=True
                )
                img_20 = F.interpolate(
                    self.img, scale_factor=2.0, mode="bilinear", align_corners=True
                )
            ...
            with torch.no_grad():
                ...
                img_sam = F.interpolate(
                    denorm(self.img) * 255,
                    (self.size_sam, self.size_sam),
                    mode="bilinear",
                    align_corners=True,
                )
                ...
                for i in range(B):
                    ...
                    for ct in self.label[i].nonzero(as_tuple=False)[:, 0]:
                        ...
                        peak_max = torch.cat(
                            (coord_w.view(1, 1), coord_h.view(1, 1)), dim=-1
                        )  # (1,2)
                        ...
                        cam_filtered = ndi.maximum_filter(
                            cam_target_np, size=3, mode="constant"
                        )
                        peaks_temp = peak_local_max(cam_filtered, min_distance=20)
                        peaks_valid = peaks_temp[
                            cam_target_np[peaks_temp[:, 0], peaks_temp[:, 1]]
                            > self.th_multi
                        ]

                        # Aggregate all the peaks
                        peaks = np.concatenate(
                            (peak_max, peaks_valid[1:]), axis=0
                        )  # (NP,2)
                ...
                for i in range(B):
                    for k in points_all[i].keys():
                        ...
                        target_conf = F.interpolate(
                            target_conf, (H, W), mode="bilinear", align_corners=False
                        )[0, 0]
        ...
        cam_main = F.interpolate(
            cam_main, size=(H, W), mode="bilinear", align_corners=False
        ) * self.label.view(B, C, 1, 1)
        ...
        feat_main = F.interpolate(
            feat_main, size=(H, W), mode="bilinear", align_corners=False
        )
        ...
        with torch.no_grad():
            pred_labels = (
                pred_main > 0.5
            ).long()  # Convert logits to binary predictions (assuming binary classification)
    ...
    def infer_init(self):
        ...
        self.net_main_replicas = torch.nn.parallel.replicate(
            self.net_main.module, list(range(n_gpus))
        )
    ...
    def infer_multi(
        ...
        def _work(i, img):
            with torch.no_grad():
                with torch.cuda.device(i % n_gpus):
                    ...
                    cam = out["cam"]
                    cam = F.interpolate(
                        cam, (H, W), mode="bilinear", align_corners=False
                    )[0]
                    cam = F.relu(cam)
        ...
        thread_pool = pyutils.BatchThreader(
            _work, list(enumerate(self.img)), batch_size=8, prefetch_size=0, processes=8
        )
        ...
        if crf:
            for a in self.args.alphas:
                ...
                np.save(
                    osp.join(crf_path, str(a).zfill(2), self.name + ".npy"), crf_dict
                )
        ...
        for i in range(len(self.acc_names)):
            if self.right_count[i] != 0:
                acc = (
                    100
                    * self.right_count[i]
                    / (self.right_count[i] + self.wrong_count[i])
                )
