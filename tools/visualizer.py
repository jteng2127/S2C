...
class Visualizer:
    ...
    def plot_loss(self, epoch, losses):
        ...
        self.vis.line(
            X=np.stack(
                [np.array(self.data_loss["X"])] * len(self.data_loss["legend"]), 1
            ),
    ...
    def plot_acc(self, epoch, accs):
        ...
        self.vis.line(
            X=np.stack(
                [np.array(self.data_acc["X"])] * len(self.data_acc["legend"]), 1
            ),
    ...
    def plot_mask(
        ...
        self.vis.images(
            imgs_remain,
            opts=dict(title="image remain", caption=remain_preds),
            env="mask",
        )
        self.vis.images(
            mask_remain, opts=dict(title="mask remain", caption=labels), env="mask"
        )

    def plot_mask_gpp(
        self, imgs, cams, I_P, t_H, t_L, labels, all_preds, remain_preds, reset=True
    ):
        if reset:
            self.vis.close(env="mask")
        # self.vis.text(labels, env="mask")
        ...
        self.vis.images(
            t_H, opts=dict(title="image remain", caption=remain_preds), env="mask"
        )
    ...
    def plot_mask_recon(
        ...
        if reset:
            self.vis.close(env="mask")
        self.vis.images(
            imgs, opts=dict(title="image", caption=original_preds), env="mask"
        )
        self.vis.images(cam, opts=dict(title="cam", caption=labels), env="mask")
        self.vis.images(
            imgs_recon_masked,
            opts=dict(title="image", caption=original_preds),
            env="mask",
        )
        self.vis.images(cam_recon, opts=dict(title="cam", caption=labels), env="mask")
        self.vis.images(
            imgs_recon_fg, opts=dict(title="image", caption=fg_preds), env="mask"
        )
        self.vis.images(
            cam_recon_fg, opts=dict(title="cam", caption=labels), env="mask"
        )
        self.vis.images(
            imgs_recon_bg, opts=dict(title="image", caption=bg_pred), env="mask"
        )
        self.vis.images(
            cam_recon_bg, opts=dict(title="cam", caption=labels), env="mask"
        )
