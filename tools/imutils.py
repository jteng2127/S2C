...
class random_crop:
    ...
    def __call__(self, imgarr, sal=None, get_xy=False, xy=None, seg=True):
        ...
        if not xy or not seg:
            container = np.zeros(
                (self.cropsize, self.cropsize, imgarr.shape[-1]), np.float32
            )
        else:
            container = (
                np.ones((self.cropsize, self.cropsize, imgarr.shape[-1]), np.float32)
                * 255
            )
...
def denorm(img):
    ...
    tf_denorm = transforms.Normalize(
        mean=[
            -mean_img[0] / std_img[0],
            -mean_img[1] / std_img[1],
...
def norm(img):
    ...
    tf_norm = transforms.Normalize(
        mean=[mean_img[0], mean_img[1], mean_img[2]],
...
def norm_vgg(img):
    ...
    tf_norm = transforms.Normalize(
        mean=[mean_img[0], mean_img[1], mean_img[2]], std=[1, 1, 1]
    )
...
def _crf_with_alpha(cam_dict, img, alpha=10):
    orig_img = np.ascontiguousarray(
        np.uint8(
            Image.open(
                os.path.join(os.path.join(VOC2012_DIR, "JPEGImages"), img + ".jpg")
            )
        )
    )
    mean_img = [0.485, 0.456, 0.406]
...
def _crf_with_alpha_rrm(ori_img, cam_dict, alpha):
    ...
    crf_score = crf_inference(
        np.ascontiguousarray(ori_img), bgcam_score, labels=bgcam_score.shape[0]
    )
...
def crf_dl(name, probs, t=10, scale_factor=1, labels=21):
    ...
    d.addPairwiseBilateral(
        sxy=67, srgb=3, rgbim=np.copy(img), compat=4
    )  # 64.86/no normal
